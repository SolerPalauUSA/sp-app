...
function handleUrlParameters(selectedProductFromUrl, selectedSeriesFromUrl) {
  const productImage = document.querySelector(`.product-image[data-product-name='${selectedProductFromUrl}']`);
  if (productImage) {
    productImage.click();
...
document.addEventListener("DOMContentLoaded", () => {
  const selectedProductFromUrl = new URLSearchParams(window.location.search).get('product');
  const selectedSeriesFromUrl = new URLSearchParams(window.location.search).get('series');
  ...
  fetch("../data/products.json")
    ...
    .then((data) => {
      ...
      if (selectedProductFromUrl) {
        handleUrlParameters(selectedProductFromUrl, selectedSeriesFromUrl);
      }
    })
    .catch((error) => {
